...
/* Farming */
dig :- playerloc(X,Y), \+cannotbeDigged(X,Y), isAvailable(shovel), shovellevel(X), digging(X);
       playerloc(X,Y), \+cannotbeDigged(X,Y), \+isAvailable(shovel), write('Kamu tidak punya sekop untuk menggali'),nl;
       playerloc(X,Y), cannotbeDigged(X,Y), write('Kamu harus berada di tempat yang bisa digali'), nl.

digging(X) :- X==1, asserta(digloc(X,Y)), write('Kamu menggali tempat ini'), nl, addTime, decreaseStamina, stamina(Farmer, Z), nl, write('Stamina: '), write(Z), write('/12'),nl;
              X==2, asserta(digloc(X,Y)), write('Kamu menggali tempat ini'), nl, digTime(X), X==0, Z is X+1, asserta(digTime(Z), retract(digTime(X)), addTime;
              X==2, asserta(digloc(X,Y)), write('Kamu menggali tempat ini'), nl, digTime(X), X==1, asserta(digTime(0), retract(digTime(X)), addTime, decreaseStamina, stamina(Farmer, Z), nl, write('Stamina: '), write(Z), write('/12'), nl;
              X==3, asserta(digloc(X,Y)), write('Kamu menggali tempat ini'), nl, digTime(X), X==0, Z is X+1, asserta(digTime(Z), retract(digTime(X)), addTime;
              X==3, asserta(digloc(X,Y)), write('Kamu menggali tempat ini'), nl, digTime(X), X==1, Z is X+1, asserta(digTime(Z), retract(digTime(X)), addTime;
              X==3, asserta(digloc(X,Y)), write('Kamu menggali tempat ini'), nl, digTime(X), X==2, asserta(digTime(0), retract(digTime(X)), addTime, decreaseStamina, stamina(Farmer, Z), nl, write('Stamina: '), write(Z), write('/12'), nl;
...
planting(Crop) :- playerloc(X,Y), crop(Crop), season(CurrSeason), atom_concat(Crop,'_seed',Z), isAvailable(Z,Count), seasonCrop(Crop,CurrSeason), dropItem(Z,1), cropTime(Crop,Time), day(CurrDay), Z1 is Time+CurrDay, asserta(croploc(X,Y,Crop,Z1)),
                  ...
                  addTime, decreaseStamina, stamina(Farmer, Z), write('Stamina: '), write(Z), write('/12'), nl
                  retract(digloc(X,Y));
...
harvest :- playerloc(X,Y), croploc(X,Y,Crop,Time), day(CurrDay), CurrDay>=Time, addItem(Crop,1), write('Kamu memanen '), mkstr(Crop,A), write(A), write('.'), nl,
           addExpFarming(Farmer,2), addTime, decreaseStamina, stamina(Farmer, Z), write('Stamina: '), write(Z), write('/12'), nl,
           retract(croploc(X,Y,Crop,Time)),;
...
fishingByLevel(X) :- isAvailable(fishing_rod), X==1, random(1,5,Z), getFishing(Z,X), fishing(X);
                     isAvailable(fishing_rod), X==2, random(1,4,Z), getFishing(Z,X), fishing(X);
                     isAvailable(fishing_rod), X==3, random(1,3,Z), getFishing(Z,X), fishing(X);
                     \+isAvailable(fishing_rod), write('Kamu tidak punya alat pancing untuk memancing'), nl;
...
fishing(Fishing) :- Fishing==fish, random(1,6,Y), getFish(Y,Fish),addItem(Fish,1),
                    ...
                    addExpFishing(Farmer,10),nl,
                    addTime, decreaseStamina, stamina(Farmer, Z), write('Stamina: '), write(Z), write('/12'), nl;
                    Fishing==none, write('Kamu tidak mendapatkan apapun!'), nl,
                    addExpFishing(Farmer,5), nl,
                    addTime, decreaseStamina, stamina(Farmer, Z), write('Stamina: '), write(Z), write('/12'), nl.
...
ranch :- playerloc(X,Y), ranchloc(X,Y), totalChicken(A), totalSheep(B), totalCow(C), Z is A+B+C, Z=\=0,
         ...
         read(Animal), nl,
         ranching(Animal);
         playerloc(X,Y), ranchloc(X,Y), totalChicken(A), totalSheep(B), totalCow(C), Z is A+B+C, Z==0, write('Kamu tidak punya Hewan di peternakan').
